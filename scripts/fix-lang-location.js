// Fix: add .html to location.href='slug?lang=...' language-toggle links
// when slug.html exists in the same directory.
const fs = require('fs');
...
for (const f of files) {
  ...
  let changedCount = 0;
  const updated = txt.replace(re, (full, pre, url, post) => {
    const q = url.indexOf('?');
    const pathPart = url.slice(0, q);
    const query = url.slice(q);
...
console.log('TOTAL location links fixed:', fixedTotal);
