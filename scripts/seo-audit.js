...
for (const f of files) {
  ...
  if (/^isomerism\\/.test(norm)) continue; // legacy top-level isomerism (orphaned)
  if (norm === 'isomerism.html' || norm === 'isomerism.pt.html') continue;
  ...
  else if (title.trim().length < 5) problems.push('TITLE BLANK');
  ...
  if (h1s === 0) problems.push('H1=0');
  if (!hreflang) problems.push('NO HREFLANG');
  if (problems.length) {
    issues++;
    out.push(f + ' :: ' + problems.join(', '));
  }
}
out.push('PAGES with issues: ' + issues);
