...
assert.equal(logic.labToolFromQuery('?tool=equilibrium'), 'equilibrium');
assert.equal(logic.labToolFromQuery('?tool=acid-base'), 'acid-base');
assert.equal(logic.labHref('equilibrium'), '/app?section=pro-lab&tool=equilibrium');
...
assert.equal(logic.uxError({ status: 400, code: 'duplicate_element' }).kind, 'solver');
assert.equal(logic.uxError({ status: 400, code: 'invalid_given_species' }).bodyKey, 'errInvalidGivenSpecies');
...
assert.match(authApp, /SITE_NAV/);
assert.match(authApp, /\/periodic-table\.html/);
assert.match(authApp, /\/viewer\/atomic-models\.html/);
assert.match(authApp, /navGroupSite/);
...
assert.match(authApp, /Balance reactions and solve stoichiometry step by step/);
...
assert.match(proLab, /labHref\('reactions'\)/);
assert.match(proLab, /labHref\('formula'\)/);
assert.match(proLab, /labHref\('solutions'\)/);
assert.match(proLab, /labHref\('equilibrium'\)/);
assert.match(proLab, /labHref\('acid-base'\)/);
assert.doesNotMatch(proLab, /locked \? '\/pricing'/);
...
assert.match(calcs, /tool=equilibrium/);
assert.match(calcs, /tool=acid-base/);
assert.match(calcs, /data-target="equilibrium"/);
const loginHtml = readFileSync(new URL('../login.html', import.meta.url), 'utf8');
assert.doesNotMatch(loginHtml, /auth-signup-name/);
assert.doesNotMatch(loginHtml, /auth-signup-username/);
assert.doesNotMatch(loginHtml, /Matheus Stan Gherlin/);
assert.doesNotMatch(loginHtml, /placeholder="matheusstan"/);
...
const destIdx = css.indexOf('.ws-dest-grid {');
const destMediaIdx = css.indexOf('.ws-dest-grid { grid-template-columns: 1fr; }');
assert.ok(destIdx !== -1 && destMediaIdx !== -1 && destIdx < destMediaIdx);
...
assert.match(appHtml, /Explore the Atomurus study workspace/);
...
assert.match(pricing, /ADVANCED CHEMISTRY SOLVERS/);
assert.match(pricing, /Reaction balancing/);
assert.match(pricing, /Chemical equilibrium/);
