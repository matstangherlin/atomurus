...
assert.match(authApp, /ws-account-tabs/);
assert.match(authApp, /workspaceNav/);
...
assert.match(appHtml, /Your Atomurus workspace/);
assert.match(appHtml, /data-atomurus-account/);
assert.match(appHtml, /assets\/pro-features\.js/);
...
assert.match(pricing, /3D Atomic Models, Molecules, Allotropes and Isomerism/);
assert.match(pricing, /Molar mass, dilution, scientific, unit, gas and pH calculators/);
assert.match(pricing, /Free account: library, notes, Study Sets and flashcards/);
assert.doesNotMatch(pricing, /Extra calculators with a free account/);
assert.doesNotMatch(pricing, /3D molecule viewer, atomic models, allotropes and isomerism/);
